...
context.task(
	...
	run:  fn (self build.Task) ! {
		...
		rename('./docs/html/${project_name}.html', './docs/html/index.html')!
	}
)
...
context.task(
	name: 'symlink'
	help: 'Create a symlink to the ${project_name} library'
	run:  fn (self build.Task) ! {
		cwd := os.getwd()
		modules_dir := os.vmodules_dir()
		symlink_path := '${modules_dir}/${project_name} '
